...
class PunchOutViewModel @Inject constructor(
    ...
    init {
        button2Enabled.addSource(imageUrl1) {
            button2Enabled.value = it.isNotEmpty() && village.value!!.isNotEmpty()
        }

        button2Enabled.addSource(village) {
            button2Enabled.value = it.isNotEmpty() && imageUrl1.value!!.isNotEmpty()
        }
